...
## For 2D physics bodies use [AreaTP2D] instead.
...
signal teleport(body: Node3D)

## The destination path where the entity will be teleported.
## Enforced via the inspector to point to a [Marker3D].
@export_custom(PROPERTY_HINT_RESOURCE_TYPE, "SceneNodePath:Marker3D")
