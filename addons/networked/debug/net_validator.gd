...
##
## Validators plug into [method NetworkedDebugReporter._execute_validators] and are
## invoked in phase order. If any validator in a phase returns errors, later phases
## are skipped — preventing cascading failures (e.g. heuristic checks on a
...
##
## [b]Note:[/b] The validator pattern is scoped to topology-style checks that return
## plain error strings. Detection logic that requires structured output (race data,
## zombie scans) lives in dedicated reporters, not validators.
...
## Run only when structural checks pass. Logical checks verify runtime invariants
## (e.g. authority assignments, config consistency).
const LOGICAL: int = 100

## Run only when logical checks pass. Heavy heuristics may traverse large node trees
## or perform expensive comparisons.
