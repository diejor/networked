...
##
## Opens cross-frame call stacks ([NetwSpan] / [NetwPeerSpan]) and tracks which
## span is currently active so [DebugReporter] can attach C++ errors
## to the right operation.
## [br][br]
## All state lives in static variables that persist for the process lifetime.
## [DebugReporter] calls [method reset] at the start of each debug
## session to clear any stale spans from a previous run.
## [br][br]
## [b]Usage:[/b]
## [codeblock]
...
## [br][br]
## Returns a no-op [NetwSpan] (empty [member NetwSpan.id]) when the debugger
...
static func begin(
	...
	if not message_delegate.is_valid():
		return NetwSpan.new(&"", span_label)
...
## [br][br]
## Returns a no-op [NetwPeerSpan] when the debugger is not active.
static func begin_peer(
	...
	if not message_delegate.is_valid():
		return NetwPeerSpan.new(&"", span_label)
...
static func active_span() -> RefCounted:
	...
	while i >= 0:
		var s = _active[i]
		if s.state == 0: # NetwSpan.State.OPEN
			return s
		i -= 1
	return null


## Returns the most recently opened peer-aware span that is still open, or
## [code]null[/code].
static func active_peer_span() -> RefCounted:
	var i := _active.size() - 1
	while i >= 0:
		var s = _active[i]
		if s.has_method(&"affects") and s.state == 0: # NetwSpan.State.OPEN
			return s
		i -= 1
	return null
...
# [br][br]
# Called automatically by [method NetwSpan.end] and [method NetwSpan.fail].
