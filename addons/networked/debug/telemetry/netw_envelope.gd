...
## Creates a [NetwEnvelope] from a [MultiplayerTree] and message data.
static func from_mt(
		mt: MultiplayerTree,
		p_msg: StringName,
		data: Dictionary,
	...
	var e := NetwEnvelope.new()
	e.source_path = str(mt.get_path()) if mt.is_inside_tree() else str(mt.name)
	e.reporter_id = rid

	if mt.api \
			and mt.api.has_multiplayer_peer() \
			and mt.multiplayer_peer != null \
			and mt.multiplayer_peer.get_connection_status() \
					!= MultiplayerPeer.CONNECTION_DISCONNECTED:
		e.peer_id = mt.api.get_unique_id()
	else:
		e.peer_id = 0
...
## Creates a [NetwEnvelope] from a [Dictionary].
static func from_dict(d: Dictionary) -> NetwEnvelope:
	var e := NetwEnvelope.new()
