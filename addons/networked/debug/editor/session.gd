...
#        ┠╴tree_name (String)
#        ┠╴display_name (String)
#        ┠╴role ([enum NetwMultiplayer.Role])
#        ┠╴is_server (bool)
...
func _route_envelope(envelope: NetwEnvelope, is_remote: bool = false) -> void:
	...
	match envelope.msg:
		"networked:session_registered":
			_on_session_registered(envelope, is_remote)
		"networked:session_unregistered":
			_on_session_unregistered(envelope, is_remote)
		"networked:peer_connected":
			_on_peer_event(envelope, true)
		"networked:peer_disconnected":
			_on_peer_event(envelope, false)
		"networked:clock_sample":
			_on_clock_sample(envelope, is_remote)
		"networked:crash_manifest":
			_on_crash_manifest(envelope)
		"networked:span_open":
			_on_span(envelope, "open")
		"networked:span_step":
			_on_span(envelope, "step")
		"networked:span_close":
			_on_span(envelope, "close")
		"networked:span_fail":
			_on_span(envelope, "close")
		"networked:span_step_warn":
			_on_span(envelope, "close")
		"networked:lobby_event":
			_on_lobby_event(envelope)
		"networked:topology_snapshot":
			_on_topology_snapshot(envelope)
		_:
...
func _on_session_registered(envelope: NetwEnvelope, is_remote: bool = false) -> void:
	...
	if pk in _peers:
		...
		if _peers[pk].get("role", NetwMultiplayer.Role.NONE) != role:
			_peers[pk]["role"] = role
...
func _on_session_unregistered(envelope: NetwEnvelope, is_remote: bool = false) -> void:
	...
	if is_remote and plugin:
		...
		plugin.send_to_game(
			session_id,
			"networked:remote_session_unregistered",
			[envelope.to_dict()],
		)
...
func _role_from_payload(d: Dictionary, peer_id: int) -> NetwMultiplayer.Role:
	if d.has("role"):
...
func _role_is_host(role: NetwMultiplayer.Role) -> bool:
	return role == NetwMultiplayer.Role.DEDICATED_SERVER \
			or role == NetwMultiplayer.Role.LISTEN_SERVER
