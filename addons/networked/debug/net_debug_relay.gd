...
## (the game multiplayer context) rather than the default
## [code]/root[/code] [SceneMultiplayer].
## [br][br]
## [b]Design:[/b] pure stateless byte pipe. No deserialization, no caches,
## no history.
## [br][br]
## [b]Single inbound RPC:[/b] [method forward_to_relay].
## The relay determines internally whether to deliver to the local
## [EngineDebugger] by checking [method _is_local_peer] on the sender —
## same-process clients already sent directly via [EngineDebugger] and must not
## be echoed. The reporter always calls the same RPC regardless of topology.
## [br][br]
## [b]Dedup sites in this file:[/b]
## [br]- [b]DEDUP-1[/b] — [method dispatch_to_recipients]: skip local peers on
## fan-out.
## [br]- [b]DEDUP-2[/b] — [method register_as_recipient]: one slot per
## [param reporter_id].
@tool
...
## Returns [code]true[/code] if the relay is currently connected and active.
...
## Forwards server-originated [param envelope_bytes] to all registered remote
## recipients.
## [br][br]
...
## Called by all clients regardless of whether they share an OS process with
## the relay.
## [br][br]
...
func forward_to_relay(envelope_bytes: PackedByteArray) -> void:
	...
	# DEDUP-1 (local echo): same-process clients already delivered via
	# EngineDebugger.
	if EngineDebugger.is_active() and not _is_local_peer(sender):
		EngineDebugger.send_message(
			"networked:envelope_remote",
			[envelope_bytes]
		)
...
## RPC that forwards an envelope to a specific peer's debugger.
...
## Registers a remote peer as a debug recipient.
@rpc("any_peer", "call_remote", "reliable")
func register_as_recipient(token: String = "", reporter_id: String = "") -> void:
	...
	if sender == 0 or sender == 1:
		# Server's own editor uses the direct EngineDebugger path.
		return

	var expected: String = ProjectSettings.get_setting(
	...
	if sender not in NetworkedDebugger._process_recipients:
		Netw.dbg.info(
			"Relay: [RegisterSuccess] Registered peer %d (rid=%s) as debug recipient", 
			[sender, reporter_id]
		)
		NetworkedDebugger._process_recipients.append(sender)
...
## Unregisters a peer from the relay's recipient list.
...
func _valid_sender(sender: int, byte_size: int) -> bool:
	...
	if byte_size > MAX_PAYLOAD_BYTES:
		Netw.dbg.error(
			"Relay: [DropOversized] %d bytes from peer %d" % [byte_size, sender],
			func(m): push_error(m)
		)
		return false
	return true
...
func _fanout(envelope_bytes: PackedByteArray, except_sender: int) -> void:
	for rid: int in NetworkedDebugger._process_recipients:
		if rid != 1 and rid != except_sender and \
				multiplayer.get_peers().has(rid):
			forward_to_peer.rpc_id(rid, envelope_bytes)
