...
func _ready() -> void:
	if not ProjectSettings.get_setting("debug/file_logging/enable_file_logging", false):
		NetLog.error("ErrorWatchdog: 'debug/file_logging/enable_file_logging' is OFF — enable it in Project Settings → Debug → File Logging.", [], func(m): push_error(m))
		return
...
func _tail_log() -> void:
	...
	if not file:
		NetLog.error("ErrorWatchdog: could not open log file: %s", [_log_path], func(m): push_error(m))
		return
