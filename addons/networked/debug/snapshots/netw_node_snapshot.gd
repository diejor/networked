...
##
## Created via [method from_node] and attached to [NetwTopologyManifest] so the
...
static func from_node(node: Node) -> NetwNodeSnapshot:
	var snap := NetwNodeSnapshot.new()
	...
	if node.has_method("_get_net_debug_state"):
		...
		if raw is Dictionary:
			...
			push_warning(
				"NetwNodeSnapshot: %s._get_net_debug_state() must return " +
				"Dictionary, got %s" % \
...
static func _sanitize(d: Dictionary) -> Dictionary:
	...
	for k: Variant in d:
		...
		if typeof(v) in [TYPE_OBJECT, TYPE_RID, TYPE_CALLABLE, TYPE_SIGNAL]:
			push_warning(
				"NetwNodeSnapshot: _get_net_debug_state() key '%s' has " +
