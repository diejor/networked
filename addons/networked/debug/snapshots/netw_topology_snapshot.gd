...
class SyncInfo:
	...
	## Array of [NetwTopologySnapshot.PropInfo].
...
var role: NetwMultiplayer.Role = NetwMultiplayer.Role.NONE
var is_server: bool = false

## Cache diagnostic data: [code]{"hit": bool, "hooked": bool}[/code].
var cache_info: Dictionary = { }

## Array of [NetwTopologySnapshot.SyncInfo].
...
func to_dict() -> Dictionary:
	...
	return {
		...
		"role": role,
		"role_name": NetwMultiplayer.Role.keys()[role],
		"is_server": is_server,
...
## Creates a [NetwTopologySnapshot] from a [Dictionary].
static func from_dict(d: Dictionary) -> NetwTopologySnapshot:
	...
	if d.has("role"):
		snap.role = d.get("role", NetwMultiplayer.Role.NONE)
	elif d.get("is_server", false):
		snap.role = NetwMultiplayer.Role.DEDICATED_SERVER
	else:
		snap.role = NetwMultiplayer.Role.CLIENT
