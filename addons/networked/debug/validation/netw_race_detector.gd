## Detects network races in Godot's Multiplayer API.
##
## An instance so the detection path is steppable (no statics, per the debugger's
## no-statics invariant). Owned by [SimplifyPathRaceValidator].
## [br][br]
