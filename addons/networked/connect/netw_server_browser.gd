## The browse model behind a server list: what is out there, and what shape it
## is in.
##
## A browser is a client of the session, never a peer of it. It probes saved
## targets through the session's transports, reads lobby lists from every
## [LobbyDirectory] registered on the session, and classifies each result
## against the local [member NetwSessionHandle.app_id]. It depends on none of
## [NetwSessionHandle], [NetwConnectAttempt], or any UI, so it [i]cannot[/i]
## start a connection, which is the invariant that keeps it a model: picking a
## row and joining it are two different objects' jobs.
## [codeblock]
## var browser := NetwServerBrowser.new(Netw.of(self))
## browser.target_updated.connect(_on_status)
## browser.load_server_list()   # restore saved direct servers
## browser.refresh()            # probe saved + list directories
##
## # The session, not the browser, does the joining.
## await NetwConnector.of(Netw.of(self)).join(browser.targets[0], payload)
## [/codeblock]
##
## Directories bind themselves. The browser reads
...
# The session supplies the transports probing resolves against, the registered
# directories, and the app tag results are classified with. Weakly held, so a
# browser never extends the lifetime of the session it browses for.
...
## The session this browser browses for, while it remains live.
...
func _run_probe(target: NetwConnectTarget) -> void:
	var result: NetwProbeResult = await _probe(target)
	_probe_active -= 1
...
func _local_app_id() -> String:
	var a := api()
	return String(a.session.app_id) if a else ""
...
## The registered [NetwTransport]s available on this platform, offered by a
## connect form's Add and Host dialogs.
...
## The registered [NetwTransport]s that can also host here, offered by a Host
...
# The transports this browser resolves against: whatever the session resolves
# against when there is one, and the process-global registry otherwise, so a
# browser with no session still lists what this build can run.
func _transports() -> Array[NetwTransport]:
	var a := api()
	return NetwConnector.of(a).transports_in_effect() if a else NetwTransport.registered()


# Reads a target through the transport that recognizes it. A scheme no
# transport claims reports unsupported rather than failing.
