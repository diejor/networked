...
func _initialize_backend() -> void:
	...
	_initialized = true
	if not backend:
		Netw.dbg.error("NetworkedDatabase: no backend assigned. Calls will be no-ops.", func(m): push_error(m))
		return
	var err := backend._initialize(_schema)
	if err != OK:
		Netw.dbg.error("NetworkedDatabase: backend initialization failed. Error: %s" % [error_string(err)], func(m): push_error(m))
...
func transaction(body: Callable) -> Error:
	if not backend:
		Netw.dbg.error("NetworkedDatabase: transaction called but no backend is set.", func(m): push_error(m))
...
func find_by_id(table: StringName, id: StringName, out_error: Array = [OK]) -> Dictionary:
	if not backend:
		Netw.dbg.error("NetworkedDatabase: find_by_id called but no backend is set.", func(m): push_error(m))
		out_error[0] = ERR_UNCONFIGURED
		return {}
...
func find_all(table: StringName, filter: Dictionary = {}) -> Array[Dictionary]:
	if not backend:
		Netw.dbg.error("NetworkedDatabase: find_all called but no backend is set.", func(m): push_error(m))
		return []
