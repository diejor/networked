...
## facade no-ops, so a client-authoritative game carries nothing.
...
##
## [br][br]Resolves the backing [LagCompensation] node mounted under the tree, like
## [NetwInterest], so it degrades to safe empty results when no node is mounted.
class_name NetwLagCompensation
extends RefCounted

var _tree_ref: WeakRef

## Keyed optimistic effects owned by the mounted [LagCompensation] node.
...
func _service() -> LagCompensation:
	...
	if service:
		return service
	return mt.find_service_node(LagCompensation) as LagCompensation
