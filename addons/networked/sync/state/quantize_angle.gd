...
## [member bits] bits, so it decodes to [code]0 <= angle < TAU[/code] with
## no boundary discontinuity. Applies to a [float] angle.
##
## [codeblock]
## var q := NetwQuantizeAngle.new().bits(8)   # ~1.4 degree resolution
## [/codeblock]
