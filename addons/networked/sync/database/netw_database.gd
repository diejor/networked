...
class SlotEngine:
	...
	##
	## Callable only before the backend locks at init.
	...
	## Returns every save slot namespace the backend holds.
	##
	## Works before [method open] because it queries backend namespace metadata.
	...
	##
	## Works before [method open] because it deletes a backend namespace by name.
	...
	# Returns the owning database if it still exists.
	func _get_db() -> NetwDatabase:
		if not _db_ref:
			return null
		return _db_ref.get_ref() as NetwDatabase
