...
##
## A record lives at [code]<root>/<table>/<id>.<ext>[/code]. The root folds
## [member app_id] and the [member NetwDatabase.slots] namespace into
## [member base_dir].
## [codeblock]
## base_dir
## └── app_id
...
##
## [br][br]
## Every read and write goes straight to disk through [ResourceLoader] and
## [ResourceSaver]. There is no cache, so [member NetwDatabase.warm_policy] is
## ignored and a write is durable the moment it returns.
## [codeblock]
...
## Lists save-slot directories under the application directory.
##
...
## Loads one record from disk.
##
## Returns an empty [Dictionary] when no record exists.
## [codeblock]
## Dictionary
...
## Loads every record in [param table] matching [param filter].
##
## An empty [param filter] returns all records in the table directory.
## [codeblock]
## Array
## └── Dictionary
##     └── column_name (StringName)
