## Abstract storage contract for [NetwDatabase] backends.
##
## Extend this resource to implement a concrete storage strategy
## (e.g. filesystem, SQLite, Postgres). Override all methods to provide
## the storage implementation. [NetwDatabase] calls these after schema
## validation.
##
...
## names.
@abstract func initialize(schema: Dictionary) -> Error
...
## In-memory [NetwDatabaseBackend] that stores records in a nested [Dictionary].
##
