...
## Sets the optional tree used by the configured descriptor.
...
func _resolve_info() -> NetwServerInfo:
	...
	if _server_info_provider.is_valid():
		return _server_info_provider.call(api)
	var registered := Netw.resolve_server_info_provider()
	if registered.is_valid():
		return registered.call(api)
	return NetwServerInfo.from_session(_tree)
