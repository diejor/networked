## Tree-scoped interest registry and visibility service.
##
## One instance lives on [member MultiplayerTree.interest]. It owns
## pure [NetwInterestLayer] state, mirrors server mutations to clients,
## and installs one entity-side visibility filter whose verdict is the
## union of every layer the entity belongs to.
...
func _schedule_visibility_flush() -> void:
	...
	mt.call_deferred("_flush_interest_visibility")
...
func _broadcast_layer_config(layer: NetwInterestLayer) -> void:
	...
	for peer_id: int in mt.multiplayer_api.get_peers():
		mt._rpc_interest_layer_config.rpc_id(
...
func _broadcast_viewer_delta(
	...
	for target_id: int in mt.multiplayer_api.get_peers():
		mt._rpc_interest_viewer_delta.rpc_id(
...
func _broadcast_entity_delta(
	...
	for peer_id: int in mt.multiplayer_api.get_peers():
		mt._rpc_interest_entity_delta.rpc_id(
