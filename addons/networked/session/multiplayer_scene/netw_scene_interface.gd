...
## The registered declaration node, or [code]null[/code].
var manager: MultiplayerSceneManager:
	get:
		for candidate: MultiplayerSceneManager in _configs:
...
var _configs: Dictionary[MultiplayerSceneManager, NetwSceneConfig] = { }
var _scene_managers: Dictionary[MultiplayerScene, MultiplayerSceneManager] = { }
var _current_scene: MultiplayerScene
var _local_participant: NetwParticipant
var _change_request_handler: Callable
...
## Registers the declarations and active rows owned by [param manager_node].
func configure(
		manager_node: MultiplayerSceneManager,
		config: NetwSceneConfig,
) -> void:
	if not is_instance_valid(manager_node):
		return
	deconfigure(manager_node)
	_configs[manager_node] = config
	var spawned := _on_scene_spawned.bind(manager_node)
	var activated := _on_scene_activated.bind(manager_node)
	var despawned := _on_scene_despawned.bind(manager_node)
	var emptied := _on_scene_emptied.bind(manager_node)
	if not manager_node.scene_spawned.is_connected(spawned):
		manager_node.scene_spawned.connect(spawned)
	if not manager_node.scene_activated.is_connected(activated):
		manager_node.scene_activated.connect(activated)
	if not manager_node.scene_despawned.is_connected(despawned):
		manager_node.scene_despawned.connect(despawned)
	if not manager_node.scene_emptied.is_connected(emptied):
		manager_node.scene_emptied.connect(emptied)
	for active: MultiplayerScene in manager_node.active_scenes.values():
		_on_scene_spawned(active, manager_node)
	refresh_current_scene.call_deferred()


## Removes declarations and active rows owned by [param manager_node].
func deconfigure(manager_node: MultiplayerSceneManager) -> void:
	if not is_instance_valid(manager_node):
		return
	var spawned := _on_scene_spawned.bind(manager_node)
	var activated := _on_scene_activated.bind(manager_node)
	var despawned := _on_scene_despawned.bind(manager_node)
	var emptied := _on_scene_emptied.bind(manager_node)
	if manager_node.scene_spawned.is_connected(spawned):
		manager_node.scene_spawned.disconnect(spawned)
	if manager_node.scene_activated.is_connected(activated):
		manager_node.scene_activated.disconnect(activated)
	if manager_node.scene_despawned.is_connected(despawned):
		manager_node.scene_despawned.disconnect(despawned)
	if manager_node.scene_emptied.is_connected(emptied):
		manager_node.scene_emptied.disconnect(emptied)
	_configs.erase(manager_node)
	for active: MultiplayerScene in _scene_managers.keys():
		if _scene_managers.get(active) == manager_node:
...
## Infers the local participant's scene from the replicated admission rows.
func sync_local_participant() -> void:
	...
	for scene_node: MultiplayerScene in scenes.values():
		...
		if layer and layer.viewers.has(_local_participant.peer_id):
			_local_participant.current_scene = scene_node.netw_scene
			return
...
func activate(scene_ref: Variant) -> MultiplayerScene:
	...
	var manager_node := manager
	if manager_node == null:
		push_error("Cannot activate a scene without a scene declaration host.")
		return null
	if scene_ref is PackedScene:
		...
		if active:
			...
			return active
		var spawned := manager_node.spawn(packed.resource_path)
		if spawned:
			scene_activated.emit(spawned)
		return spawned
	if scene_ref is String or scene_ref is StringName:
		return manager_node.activate_scene(StringName(scene_ref))
...
func freeze(scene_name: StringName) -> void:
	...
	var manager_node := manager
	if manager_node:
		manager_node.freeze_scene(scene_name)
...
func destroy(scene_name: StringName) -> void:
	...
	var manager_node := manager
	if manager_node:
		manager_node.destroy_scene(scene_name)
...
## Sets the server policy for [method request_change].
##
## The handler receives [code](participant, scene_ref, args)[/code], where
## [code]scene_ref[/code] is a declared name from [method request_change] or a
## resource path from [method request_change_path], and returns
## [code]true[/code] to allow the request. An empty handler denies all requests.
## [br][br][b]Server Only.[/b]
func set_change_request_handler(handler: Callable) -> void:
	var api := _api()
	assert(api and api.is_server(), "Scene change policy is server-only.")
	_change_request_handler = handler
...
## [constant NetwScenePromise.Result.SUPERSEDED]. The server denies requests
## until [method set_change_request_handler] configures a policy. A request
...
# pipeline, the same double-instantiation bare-level adoption already pays.
...
func _detach_and_request(node: Node, path: String) -> void:
	_hide_and_free(node)
	# TODO: install the .on_pending placeholder as current_scene once the
	# placeholder contract is settled, so a native change fades instead of
	# blanking while the request is in flight.
	...
	_start_request(true, path, [], deadline)
...
func _on_scene_spawned(
		scene_node: MultiplayerScene,
		manager_node: MultiplayerSceneManager,
) -> void:
	if not is_instance_valid(scene_node) \
	...
	_scene_managers[scene_node] = manager_node
	sync_local_participant.call_deferred()
	refresh_current_scene.call_deferred()


func _on_scene_despawned(
		scene_node: MultiplayerScene,
		manager_node: MultiplayerSceneManager,
) -> void:
	if _scene_managers.get(scene_node) != manager_node:
		return
...
func _on_scene_activated(
	...
	if _scene_managers.get(scene_node) == manager_node:
		scene_activated.emit(scene_node)


func _on_scene_emptied(
		scene_node: MultiplayerScene,
		manager_node: MultiplayerSceneManager,
) -> void:
	if _scene_managers.get(scene_node) == manager_node:
		scene_emptied.emit(scene_node)


func _remove_scene(scene_node: MultiplayerScene) -> void:
	_scene_managers.erase(scene_node)
...
func _move_entity(
	...
	if participant:
		participant.current_scene = target.netw_scene
...
func _change_single_scene(
	...
	if target == null:
		var manager_node := manager
		if manager_node:
			manager_node._allow_single_spawn = true
		target = activate(destination)
		if manager_node:
			manager_node._allow_single_spawn = false
	if target == null:
		promise.resolve(NetwScenePromise.Result.UNAVAILABLE)
		return
...
func _receive_change_request(
	...
	if is_path:
		await _receive_path_request(
			peer_id, request_id, participant, String(scene_ref), args,
		)
	else:
		await _receive_named_request(
			peer_id, request_id, participant, StringName(scene_ref), args,
		)


# A declared-name request runs the deny-default policy handler unchanged.
func _receive_named_request(
	...
	if not _change_request_handler.is_valid() \
			or not _change_request_handler.call(participant, scene_name, args):
		_send_change_result(peer_id, request_id, NetwScenePromise.Result.DENIED)
		return
	var operation := _apply_player_change(participant, scene_name)
	if not operation.is_completed:
		await operation.completed
	_send_change_result(peer_id, request_id, operation.result)


# A path request is bounded to a real scene file, then authorized by the
# deny-default policy exactly like a named request. The mark is never consulted
# here: whether a client may reach a scene is purely the server policy's call.
func _receive_path_request(
	...
	if resolved.is_empty():
		_send_change_result(
			peer_id, request_id, NetwScenePromise.Result.UNAVAILABLE,
		)
		return
	if not _change_request_handler.is_valid() \
			or not _change_request_handler.call(participant, resolved, args):
		_send_change_result(peer_id, request_id, NetwScenePromise.Result.DENIED)
		return
	...
	if packed == null:
		...
		return
	var operation := _apply_player_change(participant, packed)
	if not operation.is_completed:
		await operation.completed
	_send_change_result(peer_id, request_id, operation.result)
...
func _apply_player_change(
	...
	participant.move_to(target.netw_scene)
...
func _config() -> NetwSceneConfig:
	for manager_node: MultiplayerSceneManager in _configs:
		if is_instance_valid(manager_node):
			return _configs[manager_node]
	return null
...
func _resolve_current_scene() -> MultiplayerScene:
	...
	if _local_participant and _local_participant.current_scene:
		return _local_participant.current_scene.unwrap()
	return null


# Relays the participant edge and refreshes the current scene.
func _on_local_scene_changed(from: NetwScene, to: NetwScene) -> void:
	local_scene_changed.emit(from, to)
	refresh_current_scene()


# Clears local presentation when the session ends.
...
func _clear_request_state() -> void:
	...
	_pending_request = null
	_pending_request_id = 0
	_change_request_handler = Callable()
