...
## When [code]true[/code], connects to [signal NetwMultiplayer.clock_on_tick] and
## emits [signal tick_snapshot] each tick. Requires a clock declared through
## [method Netw.configure_clock] on this node's session.
...
# PROCESS_MODE_ALWAYS so an ancestor held disabled by the teleport reparent
# guard never deafens _unhandled_input and strands a released key as pressed.
...
func _ready() -> void:
	...
	if tick_mode:
		...
		if clock and clock.is_configured:
			clock.before_tick.connect(_on_before_tick)
			clock.on_tick.connect(_on_tick)
		else:
			push_warning(
				"tick_mode=true but this node's session has no clock. "
				+ "Declare one with Netw.configure_clock.",
			)
...
##
## Runs at [signal NetwMultiplayer.clock_before_tick] on the controlling peer
## only, so the values the input set ships and [member NetwEntity.prediction]
## snapshots are the tick's gathered input, never a stale poll. The base is a
## no-op, so a subclass that only emits [signal tick_snapshot] needs no
## override.
## [codeblock]
## func _gather() -> void:
...
##
## An empty set is legal. A body nobody steers still carries an input
## component, because a prediction slot wires only for an entity that has both
## a state binding and an input one, so its [method _get_inputs] answers no
## actions and its command is never written.
