...
Short inline names for methods, properties, and constants should use their
role form so they become links: :ref:`connect_player() <class_MultiplayerTree_method_connect_player>`,
:ref:`OFFLINE <class_MultiplayerTree_constant_OFFLINE>`. Class names
always use the role form. Double backticks ``like this`` are reserved for
language keywords (``null``, ``true``), file names, or node names that are
not part of the API.
