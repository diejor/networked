...
$(document).ready(() => {
  ...
  // See `godot_is_latest` in Godot's conf.py. Standalone addon docs may not
  // provide Godot's custom metadata, so guard this lookup.
  const isLatestMetaElement = document.querySelector('meta[name=doc_is_latest]');
  const isLatest = isLatestMetaElement != null && isLatestMetaElement.content.toLowerCase() === 'true';
  if (isLatest) {
