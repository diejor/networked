...
| 0002 | `0002-external-godot-refs-and-abstract.patch` | Emit `:godot:` roles (resolved by `_extensions/godot_xref.py`) for types and enums that aren't in the local XML set. Adds `|abstract|` substitution for GDScript 2.0 abstract methods, guards the grouped class index against missing engine bases, and matches nested class names (`Foo.Bar.baz`) in cross-reference targets. Replaces the upstream type-based class index (Nodes/Resources/Other objects) with a **folder-driven index**: classes are grouped by their `addons/` source directory via the curated `FOLDER_GROUPS` map (recovering source paths from `class_name` declarations, since the XML omits them), with an auto-derived fallback so new files are never dropped. |
