...
``_ready`` (via :button:`Init Join Payload`) or drive it from a script with
...
because Godot calls ``_enter_tree`` top-down, and the tree mounts its
:godot:`SceneMultiplayer <SceneMultiplayer>` inside its own ``_enter_tree``.
Children running ``_ready`` later can safely read ``multiplayer`` and get the
...
- A *Client* tree with an automatically-spawned sibling *Server* tree. When
  ...
  tree duplicates itself, names the copy ``Server``, calls ``host()`` on it,
  and then calls ``join()`` on the original. Both trees end up under the
...
- ``NONE`` -- no transport assigned yet, or the session has been torn down.
- ``CLIENT`` -- this tree has joined a remote server.
- ``DEDICATED_SERVER`` -- this tree is hosting and is **not** also a player.
- ``LISTEN_SERVER`` -- this tree is hosting and is also a local player.

The role is decided during ``host()`` / ``join()`` / ``adopt_peer()`` and
...
moves from ``OFFLINE`` to ``CONNECTING`` while the backend sets up, to
``ONLINE`` once the peer is live and authentication has settled, and back
through ``DISCONNECTING`` on shutdown. The
...
.. warning::
    ...
    Reading ``is_host`` or ``is_local_client`` before the tree has finished
    configuring is a programmer error -- the role is still ``NONE`` and the
    addon will log a warning. Connect to the
    :ref:`configured <class_MultiplayerTree_signal_configured>` signal
    instead, or guard the read with ``state == State.ONLINE``.
...
to join based on the payload's ``url``. The flow looks like this:
...
3. If the URL is local **and** the backend supports embedded servers, probe
   for an existing listener (a short ``join()`` with a 1s timeout). If the
...
:godot:`SceneMultiplayer <SceneMultiplayer>`, and finalize as a ``CLIENT``
or ``LISTEN_SERVER`` based on the peer's unique ID.
...
- :ref:`server_disconnected <class_MultiplayerTree_signal_server_disconnected>`
  ...
  already moved back toward ``OFFLINE`` by the time the handler runs, so it
  is safe to immediately call ``disconnect_player()`` or change scenes.
...
second cap), and then resets ``state`` to ``OFFLINE`` and ``role`` to
``NONE``. If the session ran with an automatically-spawned sibling
...
``_on_exiting()`` unmounts the API, closes the peer, and clears the auth
