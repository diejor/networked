...
get used to reaching for ``Netw.ctx(self)`` first, the rest of the addon
...
contexts -- a component that resolves ``ctx.entity`` works in editor
...
``TRACE`` for "every frame is fine", ``DEBUG`` for development noise,
``INFO`` for one-line per session events, ``WARN`` for recoverable
mis-wirings, ``ERROR`` for "this session is now broken". The default
level is ``INFO``; switch to ``DEBUG`` while debugging join issues and you
...
context, in ``_ready`` after the surrounding tree is configured:
