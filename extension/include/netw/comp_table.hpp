...
class NetwCompTable : public godot::RefCounted {
    GDCLASS(NetwCompTable, godot::RefCounted)
...
public:
    ...
    bool get_poisoned() const { return poisoned; }
    void set_poisoned(bool p_poisoned) { poisoned = p_poisoned; }
    int64_t get_table_hash() const { return table_hash; }
    void set_table_hash(int64_t p_hash) { table_hash = p_hash; }
    int64_t get_wire_hash() const { return wire_hash; }
    void set_wire_hash(int64_t p_hash) { wire_hash = p_hash; }
...
VARIANT_ENUM_CAST(netw::NetwCompTable::Address);
