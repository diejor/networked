...
public:
    ...
    void set_snap_distance(double value) {
        snap_distance = value;
    }
    ...
    double get_project_age() const {
        return project_age;
    }
    bool has_snapped() const {
        return snap_taken;
    }

    godot::Ref<NetwRingBuffer> get_buffer() const {
        return buffer;
    }

    void record(int64_t tick, const godot::Variant &value, bool authoring_tick);
    void clear();
    bool is_empty() const;
