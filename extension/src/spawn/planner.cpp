...
const StringName &key_hide() {
    static const StringName name("hide");
    return name;
}
...
repl::LeaveDecision read_decision(const Dictionary &p_decision) {
    ...
    out.hide = p_decision.get(key_hide(), true);
...
const StringName &action_name(repl::SpawnAction p_action) {
    ...
    static const StringName hide("hide");
    switch (p_action) {
...
PackedInt64Array Planner::ancestry_order(
...
Array Planner::reconcile(const Array &p_rows, const PackedInt32Array &p_peers) {
