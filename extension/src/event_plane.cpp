...
// A row installed from the wire carries Strings and an emit site carries
// StringNames, because each is the cheap spelling on its own side. Variant
// equality keeps them apart, which would make a correctly spelled row match
// nothing at all.
bool same_value(const Variant &a, const Variant &b) {
...
bool holds(const Array &allowed, const Variant &value) {
    for (int index = 0; index < allowed.size(); index++) {
        if (same_value(allowed[index], value)) {
            return true;
        }
    }
    return false;
...
bool EventPlane::wants(int64_t p_event, int64_t) const {
    ...
    return hot[p_event].load(std::memory_order_relaxed) > 0;
}
...
void EventPlane::emit(const Emission &p_fact) {
    ...
    // A death knows a route and nothing else, so the identity is carried
    // forward the way the model is. A row targeting an entity by id would
    // otherwise match every event of its life except the one that ends it.
    ...
    if (armed.load(std::memory_order_relaxed)
        || hot[carried.event].load(std::memory_order_relaxed) > 0) {
...
void EventPlane::set_armed(bool p_enabled) {
    armed.store(p_enabled, std::memory_order_relaxed);
}
...
void EventPlane::rebuild_hot() {
    for (int64_t event = 0; event < EVENT_LIMIT; event++) {
        hot[event].store(0, std::memory_order_relaxed);
    }
    for (const Watch &row : rows) {
        if (!row.enabled) {
            continue;
        ...
        for (const int64_t event : row.events) {
            hot[event].fetch_add(1, std::memory_order_relaxed);
        }
    }
}
...
void EventPlane::deliver(const Emission &p_fact) {
    // The matched ids are collected before any sink runs, because a sink is
    // game code and may install or withdraw rows while it holds the frame.
    ...
    for (const int64_t id : matched) {
        ...
        // A sink holds its own copy, because a Dictionary is a reference and
        // one careless watcher would otherwise erase every other watcher's
        // history along with its own record.
