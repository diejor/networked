...
String Identity::format(const String &p_entity_id, int64_t p_peer_id) {
    NETW_ERR_COND_V(
...
StringName Identity::parse_entity(const String &p_node_name) {
...
int64_t Identity::parse_peer(const String &p_node_name) {
    const PackedStringArray parts = exactly_two_identity_parts(p_node_name);
    if (parts.size() != 2) {
        return 0;
    }
