...
#define NETW_EVENT_KEY(m_name) \
    const StringName &m_name() { \
        static const StringName held(#m_name); \
        return held; \
    }

NETW_EVENT_KEY(event)
NETW_EVENT_KEY(event_name)
NETW_EVENT_KEY(phase)
NETW_EVENT_KEY(tick)
NETW_EVENT_KEY(route)
NETW_EVENT_KEY(entity_id)
NETW_EVENT_KEY(peer)
NETW_EVENT_KEY(verdict)
NETW_EVENT_KEY(detail)
NETW_EVENT_KEY(model)

#undef NETW_EVENT_KEY

} // namespace event_key

Dictionary event_record(const EventPlane::Emission &p_fact) {
    Dictionary out;
    out[event_key::event()] = p_fact.event;
    out[event_key::event_name()] = String(EventPlane::name_of(p_fact.event));
    out[event_key::phase()] = p_fact.phase;
    out[event_key::tick()] = p_fact.tick;
    out[event_key::route()] = p_fact.route;
    out[event_key::entity_id()] = p_fact.entity_id;
    out[event_key::peer()] = p_fact.peer;
    out[event_key::verdict()] = p_fact.verdict;
    out[event_key::detail()] = p_fact.detail.duplicate();
    out[event_key::model()] = p_fact.model.duplicate();
    return out;
}
...
constexpr Named TAXONOMY[] = {
    ...
    {EventPlane::HIDDEN, "HIDDEN", "lifecycle"},
...
TypedArray<Dictionary> EventPlane::watches() const {
    TypedArray<Dictionary> listed;
...
TypedArray<Dictionary> EventPlane::ring(int64_t p_route) {
    TypedArray<Dictionary> drained;
    ...
    for (uint32_t offset = 0; offset < size; offset++) {
        ...
        drained.push_back(event_record(found->rows[at]));
    }
    rings.erase(p_route);
    return drained;
}
