...
void append_target(Array &r_out, Node *p_root, const NodePath &p_path) {
    const netw::gd::NodeProperty resolved
        = netw::gd::node_property(p_root, p_path);
...
void clear_cache(Object *p_target) {
...
TypedArray<MultiplayerSynchronizer> of_node(Object *p_target) {
    ...
    if (!target->has_meta(invalidation_meta())) {
        ...
        target->connect(
            ...
            callable_mp_static(&clear_cache).unbind(1).bind(target)
        );
    }
    return found;
}

TypedArray<MultiplayerSynchronizer> owned_by(Object *p_target) {
...
Dictionary synchronized_properties(Object *p_target) {
    Dictionary out;
    Node *target = Object::cast_to<Node>(p_target);
    if (target == nullptr) {
        return out;
...
Array governed_targets(Object *p_sync, Object *p_root) {
    Array out;
...
Array display_bindings(Object *p_sync, Object *p_root) {
    Array out;
    MultiplayerSynchronizer *sync
        = Object::cast_to<MultiplayerSynchronizer>(p_sync);
    Node *root = Object::cast_to<Node>(p_root);
...
Array spawn_state(Object *p_root, int64_t p_local_id) {
    Array out;
    Node *root = Object::cast_to<Node>(p_root);
    if (root == nullptr) {
        return out;
    ...
    while (!stack.is_empty()) {
        Node *held = stack[stack.size() - 1];
        stack.remove_at(stack.size() - 1);
        MultiplayerSynchronizer *sync
            = Object::cast_to<MultiplayerSynchronizer>(held);
        if (sync != nullptr && sync->get_replication_config().is_valid()
            && sync->get_multiplayer_authority() == int(p_local_id)
            && sync->get_node_or_null(sync->get_root_path()) == root) {
            const Ref<SceneReplicationConfig> config
                = sync->get_replication_config();
            const TypedArray<NodePath> paths = config->get_properties();
            for (int at = 0; at < paths.size(); at++) {
                const NodePath path = paths[at];
                if (!config->property_get_spawn(path)
                    || path.get_subname_count() == 0) {
                    continue;
                }
                Node *target = root;
                ...
                const NodePath subnames
                    = NodePath(String(":") + path.get_concatenated_subnames());
                Dictionary entry;
                entry[StringName("path")] = path;
                entry[StringName("value")] = gd::get_indexed(target, subnames);
                out.push_back(entry);
            }
        }
        const TypedArray<Node> children = held->get_children();
...
bool visibility_verdict(Object *p_root, int64_t p_peer, int64_t p_local_id) {
    Node *root = Object::cast_to<Node>(p_root);
    if (root == nullptr) {
        return false;
    }
    ...
    while (!stack.is_empty()) {
        ...
        if (sync != nullptr
            ...
            && sync->get_node_or_null(sync->get_root_path()) == root) {
            found = true;
            if (sync->is_visibility_public()
                || sync->get_visibility_for(int(p_peer))) {
                return true;
            }
        }
        const TypedArray<Node> children = held->get_children();
...
Variant resolve_value(Object *p_target, const NodePath &p_path) {
    Node *target = Object::cast_to<Node>(p_target);
    if (target == nullptr || p_path.is_empty()) {
        return Variant();
    }
    const netw::gd::NodeProperty resolved
        = netw::gd::node_property(target, p_path);
    if (!resolved.is_property()) {
        return Variant();
    }
...
void sync_only_server(Object *p_target) {
