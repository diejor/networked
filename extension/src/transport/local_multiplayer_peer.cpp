...
Error LocalMultiplayerPeer::send_packet(const PackedByteArray &p_buffer) {
    ...
    // The loopback is a star, not a mesh, so client-to-client traffic reaches
    // its target by way of the server.
...
void LocalMultiplayerPeer::purge_packets_from(int p_sender_id) {
    ...
    if (current_packet.peer == p_sender_id) {
...
Error LocalMultiplayerPeer::get_packet(
    ...
    // The bytes have to outlive this call, so the buffer points into the
    // packet the peer keeps rather than into a temporary.
    take_packet();
