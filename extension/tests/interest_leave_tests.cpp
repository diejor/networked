// The leave book's laws.
//
// The resolution has one asymmetry worth stating as its own case: DESPAWN is
// absorbing while RETAIN and CUSTOM are not, so a case that only ever declares
// one policy per entity cannot tell a correct resolution from one that returns
...
namespace TestNetwInterestLeave {

using namespace godot;
using netw::NetwInterestDecl;
using netw::NetwInterestEngine;
using netw::NetwInterestLeave;

constexpr int64_t ROOT = 1;
constexpr int64_t OTHER = 2;
constexpr int DESPAWN = NetwInterestLeave::DESPAWN;
constexpr int RETAIN = NetwInterestLeave::RETAIN;
constexpr int CUSTOM = NetwInterestLeave::CUSTOM;

// Constructed on first call rather than at load: a StringName built during
// static initialisation predates the table it interns into.
...
Ref<NetwInterestLeave> fresh() {
    Ref<NetwInterestLeave> book;
    book.instantiate();
    return book;
}
...
TEST_CASE(
    ...
    const Ref<NetwInterestLeave> book = fresh();

    const Ref<NetwInterestEngine> none;
...
TEST_CASE(
    ...
    const Ref<NetwInterestLeave> book = fresh();
    const Ref<NetwInterestEngine> engine = engine_with({"arena"}, DESPAWN);
    ...
    const Ref<NetwInterestEngine> retaining = engine_with({"arena"}, RETAIN);
...
TEST_CASE(
    ...
    const Ref<NetwInterestLeave> book = fresh();
    const Ref<NetwInterestEngine> engine = engine_with({"stealth"}, RETAIN);
    const Ref<NetwInterestDecl> decl = declaration();
