...
TEST_CASE(
    ...
    PackedByteArray framed
        = frame_pack(1, 0, 11, bytes({ 0xa1 }), String());
    framed = joined(
        framed,
        frame_pack(300, 3, 12, bytes({ 0xb1, 0xb2 }), String())
    );
    framed = joined(
        framed,
...
TEST_CASE(
    ...
    const PackedByteArray framed
        = frame_pack(9, 0, 21, bytes({ 4 }), String());
...
TEST_CASE(
    "[Networked][Wire][Hosted] U9 a path length varint that never terminates "
    "leaves the path empty and the body whole, rather than losing the frame"
) {
    const PackedByteArray framed = joined(
        bytes({ 1, FRAME_COMP_PATH, 3, 5 }),
        corrupt_varint()
    );
    ...
    NETW_CHECK_EQ(walk.frames[0].comp, FRAME_COMP_PATH);
    NETW_CHECK_EQ(walk.frames[0].path.is_empty(), true);
    NETW_CHECK_EQ(walk.frames[0].payload.size(), 5);
    NETW_CHECK_EQ(walk.frames[0].payload[0], 0xff);
}

TEST_CASE(
    "[Networked][Wire][Hosted] U10 a header that stops mid frame is read "
    "against a buffer that never gives out bytes it does not hold"
) {
    const FrameWalk walk = frame_unpack_all(bytes({ 0x07 }), 0);

    NETW_CHECK_EQ(walk.whole, false);
    REQUIRE(walk.frames.size() == 1);
    NETW_CHECK_EQ(walk.frames[0].route, 7);
    NETW_CHECK_EQ(walk.frames[0].comp, 0);
    NETW_CHECK_EQ(walk.frames[0].channel, 0);
    NETW_CHECK_EQ(walk.frames[0].payload.size(), 0);
}

TEST_CASE(
    "[Networked][Wire][Hosted] U11 a body length that overruns the buffer "
    "yields only the bytes that are there, and the walk is not whole"
) {
    const PackedByteArray framed
    ...
    NETW_CHECK_EQ(walk.frames[0].route, 1);
    NETW_CHECK_EQ(walk.frames[0].payload.size(), 2);
    NETW_CHECK_EQ(walk.frames[0].payload[1], 2);
}
...
TEST_CASE(
    ...
    NETW_CHECK_EQ(walk.frames[2].route, 7);
    NETW_CHECK_EQ(walk.frames[2].payload.size(), 0);
}
