...
namespace TestNetwPlayhead {

using namespace godot;
using netw::display::Decl;
using netw::display::Playhead;

Playhead playhead(int expected_interval_ticks) {
    Playhead out;
...
TEST_CASE(
    ...
    Playhead head = playhead(3);
    Decl settings;
    settings.set_param(netw::display::PARAM_FLOOR_SMOOTHING, 0.5);
    settings.set_param(netw::display::PARAM_LAG_ADAPT_RATE, 0.5);
...
TEST_CASE(
    ...
    Playhead head = playhead(3);
    Decl settings;
    settings.set_param(netw::display::PARAM_STARVATION_GRACE_FRAMES, 3);
    settings.set_param(netw::display::PARAM_LAG_ADAPT_RATE, 0.0);
    settings.set_param(netw::display::PARAM_FLOOR_SMOOTHING, 0.0);
...
TEST_CASE(
    ...
    Playhead head = playhead(3);
    Decl settings;
    settings.set_param(netw::display::PARAM_STARVATION_GRACE_FRAMES, 1);
    settings.set_param(netw::display::PARAM_STARVATION_GROWTH, 0.5);
    settings.set_param(netw::display::PARAM_MAX_EXTRA_DILATION, 0.0);
    settings.set_param(netw::display::PARAM_FLOOR_SMOOTHING, 1.0);
