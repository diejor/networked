...
SchemaRecord scalar(int type, int stride = 1) {
    SchemaRecord record;
...
SchemaRecord gridded(double low, double high, int bits) {
    SchemaRecord record;
...
TEST_CASE(
    ...
    const SchemaRecord narrow = scalar(SchemaCore::I32);
    const SchemaRecord wide = scalar(SchemaCore::I64);
...
TEST_CASE(
    ...
    SchemaRecord schema = scalar(SchemaCore::I32, 4);
    CodeRow row = CodeRow::for_plan(WirePlan::compile(schema));
    Array out;
    CHECK_FALSE(decode_scalar_row(schema, row, out));
}

TEST_CASE("[Networked][WireValue][Hosted] an empty row is not a row of zeros") {
    SchemaRecord schema = scalar(SchemaCore::I32);
    CodeRow empty;
    Array out;
...
TEST_CASE(
    ...
    SchemaRecord schema = gridded(-512.0, 512.0, 16);
...
TEST_CASE(
    ...
    SchemaRecord schema = gridded(-4.0, 4.0, 8);
    const WirePlan plan = WirePlan::compile(schema);
