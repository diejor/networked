// Laws for NetwSnapshotBook.
//
// Two questions, kept apart: a column's cadence says when it comes up, and the
// last written values say whether it moved. A book that answered only the
// first would rewrite an unchanged row every interval forever.
//
// It holds no node and reads nothing live, so every case below hands it the
// values a caller would have gathered.
...
using netw::NetwSnapshotBook;

Ref<NetwSnapshotBook> make_book(double default_interval = 0.0) {
    Ref<NetwSnapshotBook> book;
    book.instantiate();
...
TEST_CASE(
    ...
    Ref<NetwSnapshotBook> book = make_book(1.0);
    book->declare("gold", 0.5);
    book->declare("name", 0.0);

    // Half a second is one gold interval and half a name interval.
    Array due = book->advance(0.5);
...
TEST_CASE(
    ...
    Ref<NetwSnapshotBook> book = make_book();
    book->declare("gold", 0.0);

    // Never written, so the first reading is owed whatever it holds.
    Dictionary owed = book->changed(row("gold", 10));
