## Unit tests for [NetwSessionInterface] driven straight from the peer
...
class _RecordingSession extends NetwSessionInterface:
...
func _recording_api() -> NetwMultiplayer:
	...
	prior.dispose()
	api.session = _RecordingSession.new(api)
	return api
...
func test_offline_at_construction() -> void:
	var api := _bare_api()
	assert_int(api.session.state).is_equal(NetwSessionInterface.State.OFFLINE)
	assert_int(api.session.role).is_equal(NetwSessionInterface.Role.NONE)
	assert_int(api.state).is_equal(NetwSessionInterface.State.OFFLINE)
	assert_int(api.role).is_equal(NetwSessionInterface.Role.NONE)
...
func test_server_assignment_drives_online() -> void:
	var api := _bare_api()
	var peer := LocalMultiplayerPeer.new()
	peer.create_server()
	...
	assert_int(api.session.state).is_equal(NetwSessionInterface.State.ONLINE)
	assert_int(api.session.role).is_equal(NetwSessionInterface.Role.LISTEN_SERVER)


func test_dedicated_hint_resolves_dedicated_role() -> void:
	var api := _bare_api()
	var config := NetwSessionConfig.new()
	config.desired_role = NetwSessionInterface.Role.DEDICATED_SERVER
	api.session.configure(config)

	var peer := LocalMultiplayerPeer.new()
	peer.create_server()
	api.multiplayer_peer = peer

	assert_int(api.session.role).is_equal(NetwSessionInterface.Role.DEDICATED_SERVER)


func test_auth_dispatcher_arms_tree_less() -> void:
	var api := _bare_api()

	api.session.configure(NetwSessionConfig.new())
...
func test_auth_config_rearms_dispatcher_tree_less() -> void:
	...
	assert_int(await api.session.prepare_join(payload)).is_equal(OK)
	assert_bool(api.inner.auth_callback.is_valid()).is_true()
...
func test_auth_provider_prepares_and_synthesizes_host_tree_less() -> void:
	...
	var prepare_err := await api.session.prepare_join(payload)
	var peer := LocalMultiplayerPeer.new()
	peer.create_server()
	api.multiplayer_peer = peer

	assert_int(prepare_err).is_equal(OK)
	var bucket := api.get_peer_context(1).get_bucket(NetwIdentityBucket)
...
func test_client_assignment_waits_in_connecting() -> void:
	...
	assert_int(api.session.state).is_equal(NetwSessionInterface.State.CONNECTING)
...
func test_client_completes_online_on_connected_signal() -> void:
	var api := _bare_api()
	var peer := LocalMultiplayerPeer.new()
	peer.create_client(7)
	api.multiplayer_peer = peer
	var entered := [0]
	api.session_entered.connect(func() -> void: entered[0] += 1)

	# The relayed connection signal is what completes a client connect.
	api.connected_to_server.emit()

	assert_int(api.session.state).is_equal(NetwSessionInterface.State.ONLINE)
	assert_int(api.session.role).is_equal(NetwSessionInterface.Role.CLIENT)
	assert_int(api.state).is_equal(NetwSessionInterface.State.ONLINE)
	assert_int(api.role).is_equal(NetwSessionInterface.Role.CLIENT)
	assert_int(entered[0]).is_equal(1)


func test_prepared_client_auto_submits_once_on_online() -> void:
	var api := _recording_api()
	var session := api.session as _RecordingSession
	...
	assert_object(api.session._prepared_join).is_null()
...
func test_failed_preparation_never_auto_submits() -> void:
	...
	assert_int(await api.session.prepare_join(_payload(&"rejected"))) \
...
func test_cancelled_connect_discards_prepared_join() -> void:
	var api := _recording_api()
	var session := api.session as _RecordingSession
	assert_int(await api.session.prepare_join(_payload(&"cancelled"))) \
			.is_equal(OK)
	api.multiplayer_peer = _client_peer()
	api.multiplayer_peer = OfflineMultiplayerPeer.new()
...
func test_reconnect_auto_submits_newly_prepared_join() -> void:
	var api := _recording_api()
	var session := api.session as _RecordingSession
	assert_int(await api.session.prepare_join(_payload(&"stale"))).is_equal(OK)
	api.multiplayer_peer = _client_peer()
	api.multiplayer_peer = OfflineMultiplayerPeer.new()

	var replacement := _payload(&"replacement")
	assert_int(await api.session.prepare_join(replacement)).is_equal(OK)
	assert_object(api.session._prepared_join).is_same(replacement)
	api.multiplayer_peer = _client_peer()
	api.connected_to_server.emit()
...
func test_bare_client_assignment_stays_unenriched() -> void:
	...
	assert_int(api.session.state).is_equal(NetwSessionInterface.State.ONLINE)
	assert_int(session.submissions.size()).is_equal(0)
	assert_int(api.get_participants().size()).is_equal(0)
...
func test_listen_server_waits_for_explicit_join_submission() -> void:
	...
	assert_int(api.session.role).is_equal(
		NetwSessionInterface.Role.LISTEN_SERVER,
	)
	assert_object(api.get_participant(1)).is_null()
	assert_object(api.session._prepared_join).is_same(payload)

	api.session.submit_join(payload)

	assert_object(api.get_participant(1)).is_not_null()
	assert_object(api.local_participant).is_same(api.get_participant(1))
...
func test_session_pause_and_unpause_work_tree_less() -> void:
	...
	api.session.paused.connect(func(reason: String) -> void: reasons.append(reason))
	var resumed := [0]
	api.session.unpaused.connect(func() -> void: resumed[0] += 1)
...
func test_session_kick_disconnects_peer_tree_less() -> void:
	...
	api.session.kick(7, "bye")

	assert_bool(server.linked_peers.has(7)).is_false()
...
func test_auth_provider_path_auto_submits() -> void:
	...
	api.session.configure(config)
	api.session.set_auth_flow(DummyAuth.new())
	assert_int(await api.session.prepare_join(_payload(&"provider"))) \
			.is_equal(OK)
	api.multiplayer_peer = _client_peer()
...
func test_application_auth_path_auto_submits() -> void:
	...
	api.session.configure(config)
	api.session.set_auth_flow(_FailingAuth.new())
	assert_int(await api.session.prepare_join(_payload(&"application"))) \
			.is_equal(OK)
	api.multiplayer_peer = _client_peer()
...
func test_server_crash_ends_session_tree_less() -> void:
	...
	var ended := [0]
	api.session.session_ended.connect(func() -> void: ended[0] += 1)
...
func test_connected_peer_is_a_row_before_it_joins() -> void:
	...
	assert_object(api.get_participant(7)).is_null()
	assert_int(api.get_participants().size()).is_equal(0)
	...
	assert_object(api.get_participant(7)).is_not_null()
	assert_int(api.get_participants().size()).is_equal(1)
	assert_str(api.get_participant(7).username).is_equal(&"ada")
...
func test_join_flood_guard_drops_past_the_window() -> void:
	var session := NetwSessionInterface.new()

	# One honest peer stays under the window even across the resend path.
	assert_bool(session._join_flooded(7)).is_false()
	assert_bool(session._join_flooded(7)).is_false()

	# A flood from the same peer trips once the window fills.
	var tripped := false
	for i in 20:
		if session._join_flooded(7):
			tripped = true
			break
	assert_bool(tripped).is_true()

	# The host self-join carries authority and is never limited.
	for i in 20:
		assert_bool(session._join_flooded(1)).is_false()
...
func test_join_args_round_trip_through_the_session() -> void:
	...
	api.session._encode_join_args(payload)
	...
	assert_bool(api.session._decode_join_args(wire)).is_true()
...
func test_join_args_schema_mismatch_rejects() -> void:
	...
	wire.deserialize(payload.serialize())
	wire.schema_hash += 1
	assert_bool(api.session._decode_join_args(wire)).is_false()
	probe.free()
