## Coverage for [NetwDiscovery]: saved-target probing and classification, the
## concurrency cap, directory auto-bind through the service registry, and
## pure-data [NetwServerList] persistence.
class_name TestConnectKitDiscovery
...
class _FakeDirectory:
	...
	func _host_lobby(_options: LobbyDirectory.HostOptions) -> MultiplayerPeer:
		return null
...
func test_saved_target_probe_stays_ok_when_app_tags_match() -> void:
	...
	var rig := _make_discovery(transport)
	var discovery: NetwDiscovery = rig[2]

	var seen: Array = []
	discovery.target_updated.connect(func(t, r): seen.append([t, r]))
	var target := _fake_target()
	discovery.probe(target)
	await get_tree().process_frame

	assert_int(seen.size()).is_equal(1)
	var result: NetwProbeResult = discovery.get_result(target)
...
func test_probe_scheduler_caps_concurrency() -> void:
	...
	var rig := _make_discovery(transport)
	var discovery: NetwDiscovery = rig[2]
	discovery.max_concurrent_probes = 2

	# A one-element array so the lambda mutates shared state; a captured int would
	# be copied by value.
	var done := [0]
	discovery.target_updated.connect(func(_t, _r): done[0] += 1)
	for i in 5:
		discovery.probe(_fake_target(str(i)))
...
func test_directory_registered_after_construction_is_bound() -> void:
	...
	var updates: Array = []
	discovery.directory_list_updated.connect(func(id, l): updates.append(id))
	discovery.refresh()

	assert_int(discovery.targets.size()).is_equal(1)
	assert_str(discovery.targets[0].address).is_equal("7")
	assert_str(String(discovery.targets[0].scheme)).is_equal("fake")
...
func test_directory_registered_before_construction_is_adopted() -> void:
	...
	var connector := NetwConnector.new(api)
	connector.transports = [_FakeTransport.new()]
	var discovery := NetwDiscovery.new(connector)
	discovery.refresh()

	assert_int(discovery.get_discovered(&"EarlyDir").size()).is_equal(1)
...
func test_unregistering_directory_drops_its_lobbies() -> void:
	...
	assert_int(discovery.targets.size()).is_equal(0)
...
func test_saved_target_persistence_round_trip() -> void:
	var path := "user://_test_discovery_%d.tres" % Time.get_ticks_usec()
	var transport := _FakeTransport.new()
	var rig := _make_discovery(transport)
	var discovery: NetwDiscovery = rig[2]
	...
	discovery.add_target(target)
	discovery.save_server_list(path)
	assert_bool(FileAccess.file_exists(path)).is_true()

	var rig2 := _make_discovery(_FakeTransport.new())
	var reloaded: NetwDiscovery = rig2[2]
	var added: Array = []
