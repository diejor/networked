...
func before_test() -> void:
	...
	compat = api.replication._sync_compat
...
func _counter(key: String) -> int:
	return int(api.monitor_snapshot().get(key, 0))
...
func test_sync_frame_applies_and_fires_signal() -> void:
	...
	compat.handle_sync(entity, _sync_payload(0, 0, [55]), 1)
...
func test_delta_frame_applies_masked_field() -> void:
	...
	compat.handle_sync_delta(entity, _delta_payload(0, 1, [88]), 1)
...
func test_unknown_ordinal_drops() -> void:
	compat.handle_sync(entity, _sync_payload(5, 0, [1]), 1)
...
func test_non_authority_sender_drops() -> void:
	...
	compat.handle_sync(entity, _sync_payload(0, 0, [99]), 2)
...
func test_schema_hash_mismatch_poisons() -> void:
	...
	compat.note_schema(ROUTE, { 0: 0x1234 })
	var local_hash := compat._binding_of(sync).schema_hash
	var expected := (
		"NetwSyncCompat: consumed synchronizer 'Sync' (route 7) poisoned: "
		+ "schema hash %04x disagrees with the spawn descriptor 1234. " % local_hash
		+ "Its replication config must be identical on every peer."
	)

	@warning_ignore("redundant_await")
	await assert_error(func() -> void:
		compat.handle_sync(entity, _sync_payload(0, 0, [7]), 1)
	...
	compat.handle_sync(entity, _sync_payload(0, 0, [8]), 1)
	assert_int(root.synced).is_equal(0)
	assert_int(_counter("drops_sync_poisoned")).is_equal(2)
...
func test_sync_row_size_mismatch_poisons() -> void:
	...
	@warning_ignore("redundant_await")
	await assert_error(func() -> void:
		compat.handle_sync(entity, _sync_payload(0, 0, [1, 2]), 1)
	).is_push_error(expected)
...
func test_delta_mask_value_disagreement_poisons() -> void:
	...
	@warning_ignore("redundant_await")
	await assert_error(func() -> void:
		compat.handle_sync_delta(entity, _delta_payload(0, 1, []), 1)
	).is_push_error(expected)
...
func test_sync_frame_routes_through_carrier_dispatch() -> void:
	...
	var frame := NetwFrameEnvelope.pack(
		ROUTE, 0, NetwFrameEnvelope.Channel.SYNC, _sync_payload(0, 0, [21])
	)
...
func test_stale_sync_datagram_drops() -> void:
	...
	var frame_stale := NetwFrameEnvelope.pack(
		ROUTE, 0, NetwFrameEnvelope.Channel.SYNC, _sync_payload(0, 0, [17])
	)
...
func test_over_64_watch_properties_poisons_at_consumption() -> void:
	...
	@warning_ignore("redundant_await")
	await assert_error(func() -> void:
		compat.consume(over, over_sync)
	).is_push_error(expected)

	assert_bool(compat._binding_of(over_sync).poisoned).is_true()
