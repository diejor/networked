...
## frame.
##
## [method InterestCore.flush_now] has always been the escape hatch, and a test
...
func test_a_mutation_schedules_the_flush_rather_than_running_it() -> void:
	...
	assert_bool(service.participant_sees(7, entity)).is_false()
...
func test_the_flush_lands_at_the_settle_with_no_frame() -> void:
	...
	assert_bool(service.participant_sees(7, entity)).is_true()
