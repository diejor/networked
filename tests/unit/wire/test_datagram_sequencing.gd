...
func _stale_drops() -> int:
	return int(api.stats_snapshot().get("sync_drops_stale", 0))
...
func test_clear_route_resets_acceptance() -> void:
	...
	api._replication.clear_route(7)
...
func test_ungated_dispatch_bypasses_books() -> void:
	...
	api._drive_carrier(1, _prop_frame(7, &"health", 55), false)
