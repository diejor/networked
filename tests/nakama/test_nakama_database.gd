...
func before_test() -> void:
	...
	# Registered so NakamaSessionService.of resolves this configured node, not a
	# fresh default one (the descendant scan only matches scene-owned nodes).
	_tree.api.service_register(session)
...
func test_upsert_flush_then_cold_read_round_trips() -> void:
	...
	assert_int(
		await (db.backend as NakamaDatabase).drain().wait(),
	).is_equal(OK)

	# A fresh backend has a cold cache and must read from Nakama.
	var db2 := await _make_db(&"slot_a")
	var record: Dictionary = await db2.find(&"players", &"p1").wait()
...
func test_subset_upsert_preserves_untouched_columns() -> void:
	...
	assert_int(await backend.drain().wait()).is_equal(OK)
	...
	var db2 := await _make_db(&"slot_a")
	var record: Dictionary = await db2.find(&"players", &"p2").wait()
...
func test_delete_removes_record() -> void:
	...
	await db.erase(&"players", &"p3").wait()
	assert_int(await backend.drain().wait()).is_equal(OK)

	var db2 := await _make_db(&"slot_a")
	var record: Dictionary = await db2.find(&"players", &"p3").wait()
...
func test_slots_do_not_see_each_other() -> void:
	...
	assert_int(
		await (db_a.backend as NakamaDatabase).drain().wait(),
	).is_equal(OK)

	var db_b := await _make_db(&"slot_b")
	var record: Dictionary = await db_b.find(&"players", &"shared").wait()
	assert_bool(record.is_empty()).is_true()
