...
func test_lobby_directory_identity_uses_base_when_debug_suffix_disabled() -> void:
	var dir: NakamaLobbyDirectory = auto_free(NakamaLobbyDirectory.new())
	dir.device_id = "quickstart-host"
	dir.local_member_name = "diegote"
	dir.uniquify_debug_identity = false

	assert_str(dir._effective_device_id()).is_equal("quickstart-host")
	assert_str(dir._local_member_name()).is_equal("diegote")
