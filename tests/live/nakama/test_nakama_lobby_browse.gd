...
func test_public_lobby_round_trips_then_clears_on_leave() -> void:
	...
	config.server_name = "Valeria's Game"
	config.visibility = LobbyDirectory.Visibility.PUBLIC
	var peer := await host_dir._host_lobby(config)
	assert_object(peer) \
