...
## harness stand in for the shipping root install, closing the gap named in
## [code].agents/design/netw-embedding-contract-and-test-topology-analysis.md[/code]
## §2.2 (green means "the wrapped world works", never "the shipped game works").
## [codeblock]
...
## names [MultiplayerTree] or [method NetwMultiplayer.install_as_default] — only
...
const _PROBE_SCENE := preload("res://tests/conformance/probe.tscn")
...
## Declares [param scene] as the session's initial scene through this provider's
## own native declaration path — a [MultiplayerSceneManager] child under a scoped
## tree, a config added while offline under a root install — so a scenario can
## assert a declared scene comes online identically under both embeddings without
## naming either path. Must run before [method host].
func declare_initial_scene(_scene: PackedScene) -> void:
	pass


## Mounts a [MultiplayerClock] [NetwService] node under this world's host anchor,
## so a scenario can assert the service lifecycle configures the session
## identically under both embeddings. A [NetwService] resolves its session through
## the branch [NetwMultiplayer], so this must configure the host's
## [member NetwMultiplayer.clock] under a root install with no owning tree exactly
## as it does under a scoped tree.
