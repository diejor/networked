...
func test_frame_authoring_covers_zero_and_double_tick_frames() -> void:
	var scenario := PredictionScenario.new()
	await scenario.setup(self)
	var predicted := await scenario.add_predicted_entity()
	predicted.client_prediction.schedule = \
	NetwLagCompensationInterface.PredictionHandle.Schedule.FRAME
	...
	var repeat_state := predicted.client_prediction.transition_state_at(1)
	assert_dict(first_state).is_not_empty()
	assert_dict(repeat_state).is_not_empty()
	assert_that(repeat_state[&"position"]).is_not_equal(
...
func test_command_lane_deduplicates_redundancy_overlap_on_server() -> void:
	...
	predicted.client_prediction.schedule = \
	NetwLagCompensationInterface.PredictionHandle.Schedule.FRAME
	predicted.server_prediction.schedule = \
	NetwLagCompensationInterface.PredictionHandle.Schedule.FRAME
	predicted.client_root.motion = Vector2.RIGHT
	...
	assert_int(predicted.server_prediction.tape_queue_depth).is_equal(7)
