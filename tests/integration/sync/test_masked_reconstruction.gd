...
# something.
func _masked_set() -> NetwSyncSet:
	var set := NetwSyncSet.new()
	for key: StringName in [&"position", &"rotation"]:
		var field := NetwSyncSet.Field.new(key)
		field.lane = NetwSyncSet.Lane.VOLATILE
