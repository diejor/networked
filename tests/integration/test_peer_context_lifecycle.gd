...
func test_context_erased_on_peer_disconnect() -> void:
	...
	var client := harness.get_client(0)
	var client_peer_id := client.multiplayer_peer.get_unique_id()
...
func _spawn_save_player() -> void:
	var player: Node2D = await harness.join_player(
		0, TEST_LEVEL_SAVE_SCENE.resource_path, SPAWNER_PATH)
	...
	await harness.wait_for_client_player_spawn(0, LOBBY_NAME)
...
func test_server_context_does_not_contain_client_peer_id() -> void:
	...
	var server := harness.get_server()
	var client_peer_id := harness.get_client(0).multiplayer_peer.get_unique_id()
...
func test_client_context_does_not_contain_server_peer_id() -> void:
	...
	assert_that(client._peer_contexts.has(1)).is_false()
