...
func test_connected_clients_empty_initially() -> void:
	assert_that(scene.synchronizer.connected_clients).is_empty()
...
func test_registered_peer_becomes_visible() -> void:
	var client_id := client0.multiplayer_peer.get_unique_id()
	scene.synchronizer.connect_client(client_id)
...
func test_second_client_not_visible_when_first_registered() -> void:
	...
	scene.synchronizer.connect_client(id0)
...
func test_disconnect_client_adds_to_connected_clients() -> void:
	var client_id := client0.multiplayer_peer.get_unique_id()
	scene.synchronizer.connect_client(client_id)
	assert_that(scene.synchronizer.connected_clients.has(client_id)).is_true()


func test_disconnect_client_removes_from_connected_clients() -> void:
	var client_id := client0.multiplayer_peer.get_unique_id()
	scene.synchronizer.connect_client(client_id)
	scene.synchronizer.disconnect_client(client_id)
	await wait_until(func(): return not scene.synchronizer.connected_clients.has(client_id))
	assert_that(scene.synchronizer.connected_clients.has(client_id)).is_false()
