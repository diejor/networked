...
## [LagCompCore], so a scene change that frees the node leaves
## [method LagCompCore.is_configured] true and the tick loop
## still driving [method LagCompCore.tick_step].
...
func test_config_and_binding_survive_node_free() -> void:
	...
	var engine := mt.api._lagcomp
	assert_that(engine.is_configured()).is_true()
	assert_that(mt.api._clock.on_tick.is_connected(engine.tick_step)).is_true()
