...
class ActionBody extends LagCompSimBody:
	...
	func _ready() -> void:
		action = _ctx.lagcomp_action(_server_action)
...
func test_state_ready_gate_flow() -> void:
	...
	var key := s.server.api.effect_key(p.server_entity.rid, view_tick)
	...
	assert_int(p.server_prediction.stats.missing).is_greater(0)
	assert_int(s.server_sim.metrics()[&"gate_fallbacks"]).is_equal(0)
	await s.teardown()
...
func test_action_key_and_adoption_flow() -> void:
	var s := await _new_scenario()
	var p := await s.add_predicted_entity()
	var client_key := s.client.api.effect_key(p.client_entity.rid, 44)
	var server_key := s.server.api.effect_key(p.server_entity.rid, 44)

	assert_that(client_key).is_equal(server_key)

	var client_body := p.client_root as ActionBody
	var key := s.client.api.effect_key(p.client_entity.rid, s.client_clock.tick)

	client_body.fire(s.client_clock.tick)
	assert_that(client_body.get_node_or_null("Ghost")).is_not_null()
	s.client.api.effect_adopt(key)
	await get_tree().process_frame
	...
	key = s.client.api.effect_key(p.client_entity.rid, s.client_clock.tick)
