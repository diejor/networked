...
func before_test() -> void:
	var player_builder: PlayerBuilder = PlayerBuilder.new("TestPlayerMinimal")
	var _r1: PlayerBuilder = player_builder.with_spawner()
	player_packed = player_builder.pack()
	var level_builder: LevelBuilder = LevelBuilder.new("TestLevel")
	...
	var sm_factory := func() -> MultiplayerSceneManager:
		...
		sm.add_spawnable_scene(level_packed.resource_path)
		return sm
...
func test_spawned_player_is_in_scene() -> void:
	var player := harness.spawn_player(client0, player_packed)
...
func test_connect_peer_called_on_spawn() -> void:
	var _player := harness.spawn_player(client0, player_packed)
	await harness.wait_for_player(client0, &"TestLevel")
	var scene := harness.scene_on_server()
	var peer_id := client0.multiplayer_peer.get_unique_id()
...
func test_two_players_in_same_scene() -> void:
	harness.spawn_player(client0, player_packed)
	await harness.wait_for_player(client0, &"TestLevel")
	harness.spawn_player(client1, player_packed)
