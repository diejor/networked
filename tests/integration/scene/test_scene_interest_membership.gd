...
func test_admission_populates_client_layer_projection() -> void:
	...
	var client_layer := client0.api._native_core.interest_layer(layer_id)
	assert_bool(client_layer.entities.is_empty()).is_false()


func test_admission_mints_the_scene_boundary_handle() -> void:
	# One boundary is one layer: its id, its RID and its view are minted
	# together when the scene first names it, so admission and every later read
	# answer that one handle rather than a second onto the same boundary.
	var peer_id := client0.multiplayer_peer.get_unique_id()
	var scene := server_scene.entity
	var layer_id := server_api._scene_layer_id(scene)

	var minted := server_api.layer_find(layer_id)
	...
	assert_that(server_api._layer_record(minted)).is_equal(server_scene.layer)
...
func test_participant_edges_follow_the_admission_mutation() -> void:
	var peer_id := client0.multiplayer_peer.get_unique_id()
	var observed: Array[bool] = []
	var on_entered := func(_participant: NetwParticipant) -> void:
		observed.append(server_scene.admits(peer_id))
	server_scene.on_participant_entered(on_entered)

	server_scene.admit(peer_id)
	harness.spawn_player(client0, player_builder.packed)
	await harness.wait_for_player(client0, level_builder.scene_name)

	assert_that(observed).contains_exactly([true])


func test_release_mutates_admission_and_notifies_the_client() -> void:
	var peer_id := client0.multiplayer_peer.get_unique_id()
	server_scene.admit(peer_id)
	harness.spawn_player(client0, player_builder.packed)
	await harness.wait_for_player(client0, level_builder.scene_name)

	server_scene.release(peer_id)
	for _i in 2:
		client0.multiplayer.poll()

	assert_bool(server_scene.admits(peer_id)).is_false()
	assert_object(client0.api.local_participant.current_scene).is_null()
...
func test_awareness_alone_writes_the_seat_the_client_was_told() -> void:
	...
	client0.api._scene_sync_local_participant()
...
func test_awareness_infers_no_seat_for_a_scene_it_holds_no_entity_of() -> void:
	...
	var client_scenes := client0.api
	var layer_id := server_api._scene_layer_id(server_scene.entity)
	var client_layer := client0.api._native_core.interest_layer_named(layer_id)
	assert_bool(client_scenes._scene_nodes_by_label().is_empty()).is_false()
	for scene_node: Node in client_scenes._scene_nodes_by_label().values():
		client_layer.client_untrack_entity(NetwEntity.of(scene_node))
		assert_bool(client_layer.has_entity(NetwEntity.of(scene_node))) \
				.is_false()

	var participant := client0.api.local_participant
	participant.current_scene = null
	client_scenes._scene_sync_local_participant()
...
func test_state_stream_hidden_to_non_admitted_peers() -> void:
	...
	var secret_layer := service._native_core.interest_layer(&"secret_layer")
