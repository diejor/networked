...
## the same [NetwSceneInterface] verb the tier-2 door would call. Both funnel
...
## process, so the decision-table method [method NetwSceneInterface] runs on the
...
func test_server_single_active_captures_change_to() -> void:
	...
	await h.add_client()
	var scenes := h.server().api.scenes
...
func test_client_single_captures_request_and_admits_marked() -> void:
	...
	assert_object(h.server().api.scenes.scene(MARKED_SCENE_NAME)).is_not_null()
	# The capture completion signal fires so a loading screen can tear down.
	assert_array(settled).is_equal([NetwScenePromise.Result.OK])
	await h.teardown()
...
func test_client_concurrent_captures_move_me() -> void:
	...
	assert_int(promise.result).is_equal(NetwScenePromise.Result.OK)
	var server_scenes := h.server().api.scenes
	assert_object(server_scenes.scene(MARKED_SCENE_NAME)).is_not_null()
	var participant := h.server().api.participant(
...
func test_unmarked_native_change_is_not_hijacked() -> void:
	...
	assert_int(h.server().api.state).is_equal(NetwSessionInterface.State.ONLINE)
	await h.teardown()
...
func test_capture_disabled_under_multiplexed_sessions() -> void:
	...
	assert_object(h.server().api.scenes.scene(MARKED_SCENE_NAME)).is_null()
	await h.teardown()
