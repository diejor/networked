## Integration tests for [NetworkedDatabase] + [FileSystemBackend].
##
...
## once a test scene with an exported NetworkedDatabase resource exists.
...
func test_save_flow_writes_to_backend() -> void:
	...
	var record: Dictionary = db.find_by_id(&"players", &"alice")
...
func test_load_flow_reads_from_disk() -> void:
	...
	db2.register_schema(&"players", [&"position", &"health"])
	await get_tree().process_frame

	var record: Dictionary = db2.find_by_id(&"players", &"bob")
...
func test_two_entities_isolated() -> void:
	...
	var alice: Dictionary = db.find_by_id(&"players", &"alice")
	var bob: Dictionary   = db.find_by_id(&"players", &"bob")
...
func test_ghost_table_warning_on_initialize() -> void:
	...
	var err: Error = backend._initialize({&"players": [&"position"]})
...
func test_schema_mismatch_purge_in_full_flow() -> void:
	...
	var db_v1: NetworkedDatabase = auto_free(NetworkedDatabase.new())
	db_v1.backend = backend_v1
	db_v1.mismatch_policy = NetworkedDatabase.SchemaMismatchPolicy.PURGE
	db_v1.register_schema(&"players", [&"health", &"gold"])
	await get_tree().process_frame
	...
	var db_v2: NetworkedDatabase = auto_free(NetworkedDatabase.new())
	db_v2.backend = backend_v2
	db_v2.mismatch_policy = NetworkedDatabase.SchemaMismatchPolicy.PURGE
	db_v2.register_schema(&"players", [&"health"])  # 'gold' removed
	await get_tree().process_frame

	var out_err: Array[int] = [OK]
	var record: Dictionary = db_v2.find_by_id(&"players", &"charlie", out_err)
...
func test_schema_mismatch_load_partial_preserves_known_columns() -> void:
	...
	backend._upsert(&"items", &"sword", {&"damage": 15, &"rarity": 3, &"old_stat": 99})

	db.mismatch_policy = NetworkedDatabase.SchemaMismatchPolicy.LOAD_PARTIAL

	var record: Dictionary = db.find_by_id(&"items", &"sword")
...
func test_schema_mismatch_fail_returns_err_and_keeps_record() -> void:
	...
	backend._upsert(&"items", &"axe", {&"damage": 20, &"legacy_power": 5})

	db.mismatch_policy = NetworkedDatabase.SchemaMismatchPolicy.FAIL

	var out_err: Array[int] = [OK]
	var record: Dictionary = db.find_by_id(&"items", &"axe", out_err)

	assert_that(out_err[0]).is_equal(ERR_UNCONFIGURED)
	assert_that(record.is_empty()).is_true()

	# Record should NOT have been deleted (FAIL just refuses to load).
	var raw: Dictionary = backend._find_by_id(&"items", &"axe")
